...
## Nominal Subtyping のキーワード(予約語) は使ってはいけない
...
少し言い換えると、クラス名(型の世界)からインスタンス(値の世界)を生成できる、つまり値の世界と型の世界はつながっていて、リフレクションAPIを通じて行き来できるといえるでしょう。それが可能なのはコンパイル後のバイトコードに型情報が埋め込まれているからです。
